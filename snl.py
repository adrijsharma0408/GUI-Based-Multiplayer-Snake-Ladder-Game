...
def options(): #Options Menu.
  while True:
    ...
    if b5==5:
      main()
    if b1=="s":
      play(21) #In case of single player we pass 21 as the value to the play function.
    if b2==2:
      play(2)
    if b3==3:
      play(3)
    if b4==4:
      play(4)
    pygame.display.update() #Display needs to be updated whenever we want a background to appear.

def turn(score,l,s): #Accepts 3 arguments and returns 4 values.
  a=randint(1,6) #Player rolls the dice (Random number generator).
  ...
  if score<=100: #checking if the player is in the game or has won the game all-together.
    ...
    while pygame.time.get_ticks()-time<1000: #Get the below message displayed for 1500 milli seconds.
      ...
      pygame.display.update()
  return score,l,s,six #Returns final position, ladder, snake and if the player got 6 or not.
...
def play(number_of_players): #Main game play function takes the number of players as a parameter.
  ...
  xcr=381 #initial x co-ordinates of the tokens.
  ...
  while True:
    ...
    if l: #If the turn function has returns ladder to be true then we need to display this message onto the screen.
      time=pygame.time.get_ticks()
      while pygame.time.get_ticks()-time<1650:
        message_display("Oh YEAAAA!!! There is a LADDER",650,50,35)
        pygame.display.update()
    if s: #If the turn function has returns snake to be true then we need to display this message onto the screen.
      time=pygame.time.get_ticks()
      while pygame.time.get_ticks()-time<1650:
